...
| **Phase 1.5: 在线产品 Demo** | ✅ 已完成 | 把当前可运行链路包装成可持续迭代的在线产品形态 | FastAPI 包装层、Web App（Demo / Dashboard / Report）、SSE 事件流、SQLite run store、前后端同服务托管、云部署配置文档；预留 DAG / Scratchpad / Run Inspector 数据接口 |
| **Phase 2: 任务编排增强** | 🟡 部分完成 | 在不重做前端页面的前提下，增强后端任务编排和可观测数据源 | Source Layer 子里程碑已完成：SourceSeed / RawDocument / EvidenceItem 模型、SQLite source store、Evidence Index 注入、来源 API、CLI 增量索引、Official/Jina、NewsAPI、GitHub、RSS/Atom、Reddit connector；Coordinator、DAG、Scratchpad、Run Inspector、短期任务记忆仍待后续实现 |
...
Phase 1.5 已完成一次真实 E2E 验证：`/demo` 创建真实 CrewAI run，Dashboard 通过 run detail / SSE 展示事件，Report 读取真实 artifacts / sources，并验证了 `RUN_STORE_PATH` 重启持久化。验证 run 为 `273d20e1-a0d7-455a-ad39-91e336bb26fc`，终态 `needs_review`，这是 Verifier 对来源粒度的业务质检结果，不是系统链路失败。

Phase 2 当前已完成 Source Layer 子里程碑，并开始 Coordinator Foundation。Source Layer 的来源分层、刷新频率、稀疏证据综合规则和 connector 边界见 [docs/PHASE_2_SOURCE_LAYER.md](docs/PHASE_2_SOURCE_LAYER.md)；DAG / Scratchpad 基础模型和 API 见 [docs/PHASE_2_COORDINATOR_FOUNDATION.md](docs/PHASE_2_COORDINATOR_FOUNDATION.md)。

...
| 工作流 | Phase 1 当前状态 | Phase 1.5 规划 | Phase 2 规划 | Phase 3A 规划 | Phase 3B 规划 |
|--------|------------------|----------------|--------------|----------------|----------------|
| **产品体验** | Streamlit 表单化输入、阶段进度、报告预览 | 全屏对话式任务发起、报告详情页、运行历史、Dashboard 时间线；交互模型按长期产品形态设计 | 接入 DAG 节点、Scratchpad 产物、Run Inspector 下钻、任务恢复、人工复核入口 | 团队空间、权限视图、企业级审计 | 外部平台内发起任务、接收报告、处理复核通知 |
| **前端页面** | `app.py` 单体 Streamlit 页面 | React/Vite 一次性搭建产品介绍页、Demo 页、Dashboard、Report 页，并预留 DAG / evidence / inspector 数据结构 | 不重做页面，补齐真实数据绑定、错误态、空态、长任务状态和下钻视图 | 管理后台、权限配置、运行监控页 | MCP 文档页、飞书/钉钉集成配置页 |
| **后端架构** | `runner.py` + CrewAI 顺序执行 | FastAPI：`/api/runs`、`/api/runs/{id}`、`/sse/runs/{id}`、`/api/artifacts/{id}`，先包装现有真实链路 | Coordinator 主循环、工具化 Agent、DAG 状态机、Run Inspector API | 多租户任务调度、企业级权限、审计、限流 | MCP Server、飞书/钉钉机器人服务、Webhook 分发 |
| **数据与存储** | 内存结果 + Markdown 输出 | Run Store：SQLite/JSON 保存 run、event、artifact、source reference 元数据 | Scratchpad：保存 Agent 中间产物、DAG 节点状态和 provenance 细节 | PostgreSQL、Redis、对象存储、长期记忆库 | 企业知识库同步、外部平台消息与文档映射 |
| **可观测性** | 控制台日志、阶段回调、Verifier JSON | SSE 事件流、Dashboard 时间线、产物状态；事件协议预留 DAG / SemanticStep 字段 | SemanticStep、结构化事件、Run Inspector 数据源 | OpenTelemetry、指标看板、告警和审计日志 | 外部系统事件订阅、Webhook 回执、集成调用审计 |
| **部署与集成** | Streamlit Cloud / 本地运行 | FastAPI + React 同服务部署，云端可访问 | Docker Compose、环境隔离、任务队列预留 | 生产数据库、缓存、对象存储、稳定性治理 | MCP Server 发布、飞书/钉钉上线、Webhook 与企业系统对接 |
